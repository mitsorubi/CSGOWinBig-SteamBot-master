...
namespace SteamBot
{
	public class DepositTradeOfferUserHandler : UserHandler
	{
		...
		public override void OnNewTradeOffer(TradeOffer offer)
		{
			...
			string pass = File.ReadAllText (@"C:\Users\Jordan Turley\Desktop\password.txt");
			...
			if (responseJsonObj.success == 1) {
				...
				if (jsonData.minDeposit == 1) {
					...
					if (jsonData.potOver == 1) {
						...
						/* var profitTradeOffer = Bot.NewTradeOffer (new SteamID ("STEAM_0:1:30177302"));
						...
						if (itemsToKeep.Count > 0) {
							...
							doWebWithCatch (10, () => {
								if (profitTradeOffer.Send (out profitTradeOfferId, profitMessage)) { //Don't need the token because I am friends with the bot.
		...
		private void timerEvent(object CancellationTokenSource, ElapsedEventArgs e) {
			...
			if (timerTime >= 120) {
				...
				string urlTimerEnd = "http://csgowinbig.com/php/timer-end.php";
